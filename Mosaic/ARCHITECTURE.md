...
- `PoseEstimator`（`VisionPoseEstimator`）: **人物ごとに領域（矩形+15%マージン）をクロップして** `VNDetectHumanBodyPoseRequest` を実行する（Build 27〜。人物検知=シルエットの方が骨格より安定というフィードバックに基づく方式）。クロップ内で複数骨格が出た場合はシルエットマスク内の関節数が最多のものを採用し、マスク外（`PersonMaskSampler.containsNear` 緩衝0.03含む）の関節は隣接人物の写り込みとみなし除外。クロップにより人物が大きく写るため関節検出精度も向上する。関節は `PoseJoint`（正準名enum・画像正規化座標・confidence）として `PoseHint.joints` に保持。**骨格が取れない場合は顔検出起点のフォールバック**（Build 40〜）: 人物クロップ内で `VNDetectFaceRectanglesRequest`（横顔対応）を実行し、顔位置から全身領域（幅≈顔幅4.5倍・高さ≈顔高さ8.5倍）を推定してクロップし直して骨格検出を再実行する（低解像度クロップは2倍拡大してから検出）。顔検出は骨格より頑健なため、人物矩形に背景が多い・シルエットが部分的なケースの救済になる。それでも骨格が得られない人物には `HeuristicPoseEstimator` の固定比率フォールバックヒントを適用（ただしROIは関節必須のため候補は生成されない）。
