...
- アプリ上部はSF Symbolsの単一アイコンツールバーとmacOS標準メニューで、読み込み→候補生成→適用→保存の順に配置する。ファイル系は「開く」→「フォルダを一括登録」→「貼り付け」の順で隣接配置する（v0.0.00139〜）。各アイコンはツールチップとアクセシビリティラベルを持つ。キャンバスはボタン、ピンチ、スクロールによるズーム/パンに対応する。
- ライブラリ画面は「グリッド（サムネイル、サイズ変更可）」「テキストのみ」「サムネイル付きリスト」の3表示モードを切替可能。
- グリッドはサムネイル比率に合わせた密な行高と最小余白を使う。表示モードとサムネイルサイズはUserDefaultsへ保存する。
- **リスト表示（テキスト/サムネイル）は列テーブル形式**（Build 68〜）: サムネイル/ファイル名/状態/種別/解像度/ROI数/更新日時の7列（サムネイル列はサムネイル表示モードのみ表示）を横一列に並べる。種別列は「静止画」「動画」を表示する（v0.0.00138〜）。列見出しクリックでその列の内容によりソートできる（`NSTableColumn.sortDescriptorPrototype`+`tableView(_:sortDescriptorsDidChange:)`）。
- **処理済みフラグフィルタ・種別/形式フィルタ・テキスト検索フィルタ**（Build 68〜 / v0.0.00138〜）: 表示モード切替の下に「すべて/未処理/処理済」の絞り込み、種別（すべて/静止画/動画）とファイル形式（ライブラリ内の拡張子から生成）のメニューボタン、ファイル名検索を配置する。フィルタ・検索・ソートを適用した表示専用配列 `displayedLibraryItems` を一覧・選択・カーソルキー移動が参照する（`libraryItems` はフィルタの影響を受けない全件の原本のまま。一括処理・学習データ書き出し等は全件を対象に動作する）。現バージョンではセッション内のみ保持（再起動で既定に戻る）。
...
- 既存ROIのカテゴリ変更は、キャンバス上のROIを**右クリック→対象カテゴリメニュー**で行う（旧: ツールバーのカテゴリポップアップ。Build 37で生成フィルタへ置き換えたため移設）。新規手動ROIとマスクペンで作る新規レイヤは、選択中ROIまたは最後にON操作した候補カテゴリを継承する（v0.0.00153〜）。
- アニメ部位検出（イラスト/漫画）は内容ベースでカテゴリを自動付与する（§5.2）。実写のカテゴリ自動付与は骨格ベースの位置推定（乳首）のみで、鼠径部推定ROIは「その他」。
- カテゴリは検出後処理と対象形状（SAM）のプロンプトへ影響する。男性器・女性器は中心正例点＋box、それ以外はbox-onlyを使う（v0.0.00153〜）。そのためマスクキャッシュ識別子にはカテゴリと検出元を含める。
...
- 「詳細設定」ウィンドウはアイコンサイズ（小/中/大）とテキストサイズ（小/中/大）を持つ（Build 66〜）。ツールバーアイコンはSF Symbolsの `SymbolConfiguration(pointSize:)` とボタンの幅/高さ制約を動的に差し替えて見た目サイズを変更する（既定=中。小=18pt/40pt枠、中=23pt/48pt枠、大=28pt/58pt枠。Build 66で全体を一段階拡大し、既定値も大→中へ変更）。テキストサイズは `MosaicWindowController.textScale()`（小=0.8倍・中=1.0倍・大=1.25倍。Build 69で既定「中」を基準に再調整）を介して画面内のラベル・キャンバス描画テキストのフォントサイズへ反映する。静的コントロールは生成時に `applyScaledFont(_:size:weight:)` でフォント適用とレジストリ（`scaledTextControls`）登録を同時に行い、`textSizeChanged()` がレジストリを走査して全登録コントロールへ即時再適用する（Build 69〜、インスペクタ/レイヤ/ライブラリ両パネル・下部ステータス/ヘルプバー・詳細設定/画像出力ウィンドウ等を網羅）。ライブラリのテーブル/コレクションビューはセル再利用時（`NSTableView`/`NSCollectionView` の `makeView`/アイテム再利用）にもフォントを再適用するようにし、旧来の「静的な見出し等は次回起動時に反映」という制限を撤廃した。OSネイティブのツールチップ（`NSButton.toolTip`）はmacOSにフォント変更の公開APIが無いため対象外（アプリ独自のホバーヘルプ表示 `HoverHelpRelay`→ステータスバーは対象内、Build 66〜）。アイコンサイズのSymbolConfigurationは共通ヘルパ `currentIconSymbolConfiguration()`（v0.0.00125〜）で一元化し、(a) `toolbarIconButtons` 経由の通常ボタン、(b) `NSButton`経路を通らないモード切替セグメント `canvasModeControl`（`applyIconSizeToCanvasModeControl()` がセグメント画像とセグメント幅を設定へ追従）、(c) ON/OFF状態で記号を出し分ける学習モードボタン（`refreshLearningModeButton()` が画像上書き時にも同Configurationを適用）の3経路すべてが同じサイズ段階を共有する。過去に (b)(c) がこの適用から漏れて「一部アイコンだけ小さい・設定に追従しない」バグが起きたため、ツールバーへアイコン系コントロールを追加する際は必ずこのヘルパを経由すること。

## 5.13 フォルダ一括登録（リンク）と一括処理

- 「フォルダを一括登録」は選択フォルダ内の画像/動画をコピーせず**リンク**（`MosaicLibraryItem.linkedOriginalPath`）としてライブラリへ登録する（Build 54 / 動画混在はv0.0.00138〜）。静止画は `LibraryEngine.importLinked`、動画はバックグラウンドで `VideoFrameReader.loadInfo()` により解像度・尺を取得して `LibraryEngine.importLinkedVideo` へ登録する。ROI・レイヤ情報は従来どおり索引側で管理。リンクアイテムは🔗表示、参照先が無い場合は「⚠️リンク切れ」を赤字表示し、削除時も参照先の元ファイルは消えない。
- リンク切れ修正: リンク切れアイテムを選択して「リンク切れを修正」→ファイル指定の画像単位修正。未選択で実行→フォルダ指定のファイル名一致による一括修正（`repairBrokenLinks`）。
- 「一括処理」は未加工（加工後なし）かつリンク有効な全静止画を、候補生成→モザイク適用（現在の既定スタイル・マスク生成方式・対象カテゴリ設定を使用）→ライブラリ保存で連続処理する（Build 55、v0.0.00138で動画項目を対象外化）。処理はバックグラウンドで実行され、シートに処理中件数・画像名・進捗バーをリアルタイム表示し、キャンセルボタンで途中中断できる（処理済み分は保存済みのまま残る）。

## 5.15 画像出力（多形式・プレビュー付き）

- 「画像出力」ウィンドウ（Build 64〜、旧「画像を書き出す」）はリサイズ可能な専用ウィンドウで、ファイル名（旧NSSavePanelの約2倍幅）・保存先フォルダ・出力形式・形式ごとの詳細設定・全体/中央拡大プレビューを1画面にまとめる。v0.0.00138でコンテンツをウィンドウ境界から24pt離す明示制約に変更し、ラベル・ボタン・プレビューが境界へ貼り付く表示崩れを防ぐ。
...
- **モード切替アイコンとカーソル**: 編集モード=矩形破線（ROIを描くモード）、範囲選択モード=カーソル、に入替。画像上のカーソルは編集=crosshair（＋）、範囲選択=arrow で、Option(⌥)一時反転にも追従する。`flagsChanged`はファーストレスポンダにしか届かないためローカルイベントモニタで受け、`applyHoverCursor` で統一的に設定する。v0.0.00138でマスク追加ペン/消しゴム中はROI・人物・骨格レイヤのヒット状態よりペン/消しゴムカーソルを優先するようにし、レイヤ上で手のひらカーソルへ戻る不整合を防いだ。通常の編集/範囲選択ではROI上または掴める人物/骨格レイヤ上でopenHandを表示する。
...
- **AnimeSegmenterの行方向確定**: モデル出力の行方向は「入力と同一（行0=上）」とコメントで仮定していたが、実際は逆（行0=下）であることが「人物認識範囲が上下反転表示される」GUI報告で実測確定（コードレビューの「実測未検証」指摘項目）。読み出しループを上下反転。人物マスクは表示専用のため他経路への影響なし。実写経路（Vision）のマスク・矩形は`verticallyFlippedForRaster`/`normalizedRect(fromVisionRect:)`で補正済みで問題ないことを確認した。
  【v0.0.00129訂正】実写経路の`verticallyFlippedForRaster`は、表示側がrespectFlipped未指定だった時代（〜v0.0.00081）の補正で、表示側修正（v0.0.00082）後は**二重反転**になっていた（GUI報告 2026-08-06「人物マスクが上下反転表示される」。実写・動画フレームのみで発症）。実画像レンダリングで「反転を戻すと体に完全一致（SAMインスタンスマスクとのIoU: 正立0.94/反転0.44）」を確認して除去。`PersonMaskSampler`（骨格対応付け）・`maskBounds`（人物枠算出）も正立前提のため、反転マスクは骨格の人物対応付けと人物枠のy座標も誤らせていた。回帰テストは`visionPersonMaskIsNotVerticallyFlipped`（向き非依存のSAMマスクとのIoU比較）。
...
- **ツールバー再配置**: ライブラリ関連操作（リンク切れ修正・画像出力・Finderで表示）をライブラリパネルの操作行へ集約し、削除は誤操作防止のため行末尾へ。メインツールバーは ファイル系 | 一括処理→自動候補生成→適用→レイヤ削除 | 元に戻す/やり直す | モード切替 の並びに変更。v0.0.00139でファイル系は「開く」→「フォルダを一括登録」→「貼り付け」の順へ調整。名称変更:「候補を生成」→「自動候補生成」、「リンク切れを修正」→「リンク切れ修正」（`AppShortcut`レジストリ経由でメニュー・ツールチップ・ショートカット一覧へ自動反映）。
...
- **一覧表示**: 動画は🎬バッジ＋代表フレームのサムネイル（`VideoThumbnailProvider`。ディスクへは書き出さない）＋解像度欄に尺を併記する。v0.0.00138以降は状態欄を加工状態へ戻し、メディア種別は「種別」列（静止画/動画）で表示する。
...
- **入口の統一（v0.0.00135〜）**: 「画像を開く」「動画を開く」の`allowedContentTypes`は静止画（PNG/JPEG/TIFF/HEIC）と動画（MP4/MOV）の和集合で共通とし、選択後に`isVideoFile(_:)`（`URLResourceValues.contentType`による実体判定。拡張子が無い/違うファイルにも対応）で`openImageFile(at:)`／`importVideoFile(at:)`へ振り分ける。動画取り込みは`importVideoFile(at:)`へ切り出し、画像を開く／動画を開く／貼り付けの3経路で共用する（GUI報告 2026-08-10: 「画像を開く」でMOVが選べない）。
- **貼り付けはファイルURLを優先（v0.0.00135〜）**: Finderでファイルをコピーすると、ペーストボードにはファイルURLに加えて**そのファイルのアイコン画像**もNSImageとして載る。`pasteImage()`が`NSImage`だけを見ていたため、動画を貼り付けると1024×1024のMOVアイコンが静止画として登録され、動画としては一切扱われていなかった（GUI報告 2026-08-10）。URL（`urlReadingFileURLsOnly`）を先に読み、実体で振り分ける。画像ファイルの貼り付けもアイコンではなく原寸を取り込む。URLが無い場合のみ従来どおりNSImage（ブラウザ画像・スクリーンショット等）として扱う。
...
- **キーフレーム**: 「現在時刻のROI群」を`VideoEditState`へ`upsert`し、`VideoEditStore`（サイドカーJSON）へ保存する。動画本体は再エンコードしない。同一フレーム判定は既定0.01秒を維持するが、動画編集UIは元動画fpsのフレーム間隔の45%を渡し、120/240fpsの隣接フレームを別キーフレームとして保持する（v0.0.00151〜）。
...
- **出力コンテナ（v0.0.00136〜）**: 拡張子から決める（`VideoMosaicExporter.fileType(for:)`。mov/qt→MOV、m4v→M4V、既定MP4）。読み込みはMP4/MOV両対応なのに書き出しがMP4固定という非対称の解消。

## 5.34.1 動画対応 V5: 自動再検出・シーンカット・見失い時の安全側膨張（2026-08-10 v0.0.00136〜）

- **背景**: V4までは書き出し時に`VideoROITracker`を直接回すだけで、次の4点が穴になっていた。①キーフレーム間で新しく画面へ入ってきた対象は検出の機会が無く一切モザイクされない。②追跡がじわじわズレても補正されない。③カットを跨ぐと追跡は必ず破綻するが検知できない（別の似た模様へ吸着し信頼度が下がらないことがある）。④見失っても直前位置を保持するだけなので、対象が動いていればそのまま露出する。なお`VideoDetectionPipeline`（Nフレームごとの検出駆動）は実装済みだったが**アプリからは一度も呼ばれていなかった**。
- **集約先**: `MosaicVideoKit.VideoTrackingCoordinator`。キーフレーム起点の追跡に加え、自動再検出・シーンカット検出・見失い時の膨張・見失い区間の記録までを1箇所で扱う。**書き出しと追跡プレビューの双方がこれを使う**（両者で挙動が食い違うと「プレビューでは追随したのに書き出すとズレる」ことになるため）。
- **統合規則（`merge`）**: 追跡中ROIと再検出結果を IoU 0.30 以上・同一カテゴリで対応付ける。
  - 対応したもの → 検出位置へ張り直す（ドリフト補正）。ID・スタイル・グループ名は追跡側を維持する。
  - 対応しない検出結果 → 新規対象として追加する（新しいIDを振る）。
  - 検出されなかった追跡ROI → **消さない**。消すと検閲漏れになるため、常に「覆い過ぎ」側へ倒す。
  - **手描き（`source == "manual"`）は張り直さない**。ユーザーが意図して置いた範囲を検出器で上書きしない。
- **シーンカット検出**: `SceneCutDetector`が32×32グレースケールへ縮小した連続フレームの平均絶対差を見る（しきい値0.18＝画面の大部分が入れ替わった水準。パン・ズーム・人物の動きでは超えない）。カットは追跡が必ず破綻する瞬間のため、**自動再検出OFFでも検出器があればやり直す**。
- **見失い時の膨張**: 見失っているROIをフレームごとに拡大（既定0.002/フレーム・上限0.08）。**表示・描画にのみ適用し、次フレームの追跡起点は素の追跡結果のまま**にする（膨らんだ矩形を起点にすると際限なく広がる）。追跡を取り戻したフレームで拡大量は0へ戻す。
- **要確認の時間帯**: 見失いフレーム・自動追加フレームを記録し、`lostTimeRanges()`/`addedTimeRanges()`で連続区間へまとめて（5フレーム以内の途切れは同一区間）書き出し完了時にダイアログで提示する。書き出しが成功していても見失い区間はズレている可能性があるため、**黙って完了にしない**。
- **検出器の共有と直列化**: 動画の再検出は静止画と同じ`CandidateGenerationWorker`・同じ候補カテゴリ絞り込み・同じ形状適用を通す（静止画の検出精度をそのまま引き継ぐ）。ただし書き出しスレッドと静止画の候補生成が同時に走り得るようになったため、`run(_:)`を`NSLock`で直列化した（`lazy var`のモデル生成と推論セッションはスレッド安全ではない）。学習モードの候補補正（`refineCandidates`）は動画1本で学習履歴の重み付けが偏るため動画側では適用しない。
- **設定**: 詳細設定＞「動画の自動追随」（自動再検出ON/OFF・間隔10/15/30/60/90フレーム・シーンカット・見失い膨張）。既定は全てON。未設定キーは`true`として読む（`videoSettingIsEnabled`。`AppSettings.bool`は未設定で`false`を返すため、そのままでは既定ONのつもりの機能が既定OFFで動く）。
- **代償**: 再検出はONNX推論のため、書き出し時間は再検出回数ぶん延びる。間隔で調整する。

## 5.34.2 貼り付けミスで取り込まれたファイルアイコンの整理（2026-08-10 v0.0.00136〜）

- v0.0.00135以前の`pasteImage()`不具合（§5.32参照）で登録された「ファイルアイコン画像」を後から整理する導線（ファイル＞「ファイルアイコン画像を整理…」）。
- 判定は`MosaicCore.PastedIconImageDetector`の2段。①名前（`clipboard_`接頭辞）・正方形・アイコン代表サイズ（128/256/512/1024）で事前絞り込み（ライブラリ全件のデコードを避ける）。②外周4pxが一様かで確定（アイコンは絵柄が中央にあり外周が透明/単色。写真・スクリーンショットで外周が完全に一様になることはまれ）。
- **削除は取り消せないため、候補の件数と名前を提示して確認を挟む**。判定は推定であり、これ単独を削除の根拠にはしない。

## 5.34.3 動画起動導線と解析手順（2026-08-21 v0.0.00140〜）

- **動画の開き方**: ライブラリの動画項目をダブルクリック、または「元画像/動画を開く」操作で、動画を静止画ローダーへ渡さず `openVideoForEditing(_:)` へ分岐する。これにより、`.mov` を `ImageLoader` が読み込もうとして「画像を読み込めません」と出る経路を塞ぐ。再生アイコンは確認用プレビュー専用で、解析/キーフレーム編集はキャンバス下部の動画タイムラインで行う。
- **登録直後の導線**: 「画像を開く」「動画を開く」「貼り付け」で動画を登録した後は、ライブラリ登録完了後にそのままキーフレーム編集モードへ入る。プレビュー再生はライブラリの再生アイコンから明示的に開く。
- **解析手順**: 1) 動画を編集モードで開く。2) 先頭または対象時刻で「自動候補生成」を実行する。3) 必要に応じてROIを手動修正する。4) 「キーフレーム追加」で現在時刻のROIを保存する。5) タイムラインを後の時刻へ移動する。6) 「追跡を確認」でROI追随をプレビューする。7) ズレがあれば修正して再度キーフレーム追加する。8) 「動画を書き出す」でモザイク済み動画を生成する。
- **未保存処理**: 動画編集中に切替/終了時の保存確認で「保存」を選んだ場合、静止画の `saveProcessedImage` ではなく、現在時刻のROIを `VideoEditStore` のキーフレームとして保存する。動画の加工済みファイル生成は必ず `VideoMosaicExporter` の書き出し経路を使う。
- **外部アプリ上のリアルタイム処理は未実装**: 現行スコープはライブラリへ登録したローカル動画ファイルのフレーム解析・追跡・再エンコードであり、Chrome等の別アプリ内の動画表示を直接書き換えない。将来対応する場合は ScreenCaptureKit 等で対象ウィンドウ/画面を取得し、ローカル推論後に処理済みビューを別ウィンドウ/オーバーレイ/仮想カメラへ出す設計になる。DRM/HDCP保護コンテンツは取得できない、または扱えない可能性があるため、仕様上の制約として扱う。

## 5.34.4 動画編集パネルと追跡起点の厳密化（2026-08-22 v0.0.00141〜）

- **追跡起点の修正**: 追跡確認は「現在時刻に効くキーフレーム」ではなく、`VideoEditState.keyframe(before:requiringROIs:)` で「現在時刻より前のROI付きキーフレーム」を起点にする。v0.0.00140では、2つ目以降のキーフレーム上で追跡確認すると現在キーフレーム自身を起点にしてしまい、追跡区間が0秒になって処理されないように見える問題があった。
- **ログ**: 追跡確認の開始/中止/完了は `Video` カテゴリへ記録する。操作ログだけでは「ボタンは押されたが、起点なしで止まった」のような状態が追えないため、処理理由を明示する。
- **動画編集パネル**: 右サイドパネル既定で `SidePanelKind.video` を追加し、キーフレーム移動、キーフレーム追加/削除、選択/全キーフレーム削除、追跡確認、動画書き出し、動画自動モザイク処理を集約する。従来の動画下部タイムライン上の操作ボタンは移動し、下部はシーク操作に集中させる。
- **キーフレーム一覧**: `NSTableView` で No/時刻/ROI 数を表示する。ダブルクリックで該当時刻へシークし、選択行削除に対応する。削除はキーフレームのサイドカーJSONだけを変更し、動画本体は変更しない。
- **シークバー上のマーカー**: `VideoTimelineSlider` が `currentVideoEditState.keyframes` の時刻を三角マーカーとして描画する。下部のキーフレーム件数表示は廃止し、件数は動画編集パネル側へ表示する。
- **動画自動モザイク処理（v0.0.00150〜）**: 現在の候補カテゴリ/検出設定を使い、`AVAssetReader` で動画を順次デコードする。最初の検出後は全フレームでVision追跡し、詳細設定の再検出間隔で内容検出をやり直してドリフト・新規登場・シーンカットを補正する。追跡位置は毎秒約6点と再検出/見失い状態変化時をキーフレームへ保存し、停止時点までの結果と既存手動キーフレームを保持する。これは一括キーフレーム生成であり、実際のモザイク済み動画ファイルは従来どおり `VideoMosaicExporter` の書き出しで生成する。

## 5.34.5 サイドパネル高さ復元の分離（2026-08-22 v0.0.00142〜）

- **症状**: 動画編集パネル追加後、ライブラリサイドパネルが表示されず、設定初期化後も復帰しないことがある。右サイドに本来「ライブラリ」「レイヤ」「動画編集」が並ぶ構成で、先頭のライブラリが0px近くまで潰れて見えない状態になる。
- **真因**: `restoreSplitPositions()` が左右ペイン幅・サイド内高さのどれか1つでも復元すると `true` を返していたため、幅だけ復元できたケースでも「高さ復元済み」と扱われ、動画編集パネル追加前の古い高さ配分や未配分状態のまま `NSSplitView` に任せていた。
...
## 5.34.7 動画編集UI・解析停止・ROIスタイル保持（2026-08-22 v0.0.00144〜）

- **タイムライン操作**: 動画タイムラインは左から再生・一時停止・停止・シークバー・時刻表示の順にする。再生は現在時刻からフレームレートに応じた間隔で `seekVideo(to:)` を進め、停止はタイマーを破棄して先頭へ戻す。手動シーク時は再生を止め、非同期フレーム取得との競合を避ける。
- **キーフレーム状態表示**: `VideoKeyframe` に `trackingStatus` を持たせ、一覧に「追跡」列を表示する。既存JSONは欠落時 `.manual` として互換読込する。ユーザーが追跡プレビュー後に保存したキーフレームは `.tracked`、動画自動モザイク処理で生成したものは `.autoDetected` とする。
- **ROIスタイル保持**: 動画キーフレームへ保存するROIは、`style == nil` のまま全体設定を参照させず、保存・追跡・自動解析のタイミングで現在の `defaultMosaicStyle` を `MosaicROIStyle` として固定する。これにより追跡後や再読み込み後にモザイク種別・透明度・色等が既定値へ戻るデグレを防ぐ。
- **自動解析停止**: 下部ステータスバーの停止ボタンは、静止画候補生成または動画自動モザイク処理中だけ表示する。動画は `VideoMosaicExporter.CancellationFlag` と同じスレッドセーフなフラグで次サンプル処理前に停止し、停止時点までの `VideoEditState` を保存する。静止画は既存の同期的ワーカーを途中中断できないため、停止要求後に完了した結果を適用しない。
- **サイドパネル操作行**: ライブラリ/動画編集の操作アイコンは `WrappingToolbarView` でグループ単位に配置する。可能な限り1行に並べ、幅が不足した場合だけグループ境界で折り返し、同一行に複数グループが並ぶ場合はセパレータを表示する。
- **レイヤ削除**: レイヤパネルの「グループ解除」右にゴミ箱ボタンを置き、選択中のROI/ROIグループ/人物・骨格レイヤを削除できるようにする。ROI削除はUndo対象、人物・骨格レイヤ削除は従来の検出レイヤ削除と同じく表示レイヤの除去として扱う。

## 5.34.8 サイドパネル操作アイコンのコンパクト配置（2026-08-22 v0.0.00145〜）

- **コンパクト枠**: メインツールバーのアイコンサイズ設定は従来どおり維持し、ライブラリ/動画編集パネル内の操作ボタンだけ `SquareIconButton.usesCompactPanelChrome` で32pt枠に固定する。これにより、パネル内の操作アイコンが48pt以上のボタン枠でグループごとに縦へ落ちる状態を避ける。
- **折り返し計算**: `WrappingToolbarView` は横/縦間隔を2pt、セパレータを18ptへ縮小し、幅が未確定の初期計測時は全グループの自然幅で高さを測る。パネル幅が広がった場合は `setFrameSize(_:)` で intrinsic height を再計算し、縦配置が不要になったら1行へ戻す。
- **ライブラリ操作行**: ライブラリ一覧下の8操作（元画像/加工後/動画プレビュー、リンク修復/画像出力/Finder表示、データセット/削除）は通常の右パネル幅で1行に収まる寸法にし、幅不足時のみグループ単位で折り返す。

## 5.34.9 レイヤ表示/詳細トグルの折り返し配置（2026-08-22 v0.0.00146〜）

- **対象**: レイヤパネル先頭の「表示: ROI / モザイク / 人物 / 骨格」と「詳細: 輪郭 / タグ」は、サイドパネル幅が狭くなると固定横並びではチェックボックスとラベルが重なる。
- **対応**: `WrappingControlRowView` で、ラベル付きチェックボックス群を内容幅で測定し、幅不足時だけグループ単位で次行へ折り返す。`表示:` は `表示:+ROI` を同一グループにして、見出しだけが単独行にならないようにする。
- **制約**: レイヤパネルの各行はパネル幅いっぱいへ張り、幅変更時に intrinsic height を再計算する。これにより、縮小時は重ならず、拡大時は不要な複数行から1行へ戻る。
- **再修正（v0.0.00147）**: intrinsic height の再計算だけでは、縮小直後に後続の「詳細:」行が押し下がらず、チェックボックスやラベル自体が重なるケースが残った。`WrappingControlRowView` は自身の高さ制約を持ち、幅変更時に実測行数から高さを更新する。チェックボックス/ラベルは行内で折り返さずクリップ扱いにして、コントロール内の文字折返しによる縦重なりも防ぐ。

## 5.34.10 動画キーフレームの適用・プレビュー再生（2026-08-22 v0.0.00147〜）

- **症状**: 動画自動モザイク処理またはキーフレーム設定後、タイムラインを再生しても元フレームだけが表示され、モザイク済みフレームにならない。さらに動画編集中に「モザイクを適用」を押しても、再度同じキーフレームを参照するとモザイク設定が消えたように見える。
- **真因**: `seekVideo` は該当キーフレームのROIをキャンバスへ載せるだけで、モザイク済みフレームをレンダリングしていなかった。また「モザイクを適用」は静止画の加工済み画像保存経路を通っており、動画の正規保存先である `VideoEdits/<itemID>.json` へ現在時刻の `VideoKeyframe` を保存していなかった。
- **対応**: 動画編集モードのシーク/再生では、モザイク表示ONかつROIがある場合に現在フレームへ `renderMosaicOutput` を適用して表示する。保存済みキーフレームを持つ動画を開く場合はモザイク表示をONにして開始する。
- **保存ルール**: 動画編集中の「モザイクを適用」は、現在時刻のROIを保存時点のモザイク設定で固定し、`VideoEditStore` のキーフレームとして保存する。静止画の `saveProcessedImage` は呼ばない。自動処理・追跡・未保存確認保存も同じ `VideoEditState.resolvingInheritedSettings` 系の処理で、未設定ROIへ保存時点のスタイル、マスク生成方式、形状しきい値を埋める。
- **再生パフォーマンス（v0.0.00148〜v0.0.00150）**: 動画再生中は手動シークと同じ高品質経路を毎フレーム同期実行しない。専用キューで近傍フレーム取得とモザイク合成を行い、メインスレッドでは表示反映と時刻ラベル更新だけを行う。処理中に次の再生位置が来た場合は最新時刻だけを保持し、古いフレーム要求を積み上げない。v0.0.00150では再生タイマーの24fps上限を撤去し、動画fpsで実時間を進める。`AVAssetImageGenerator`、Core Imageコンテキスト、マスクエンジンを再利用し、表示解像度でデコードする。SAM指定ROIは輪郭を30表示フレームごとに再生成し、その間は追跡ROIへマスクを移動・拡縮する。動画切替・逆方向/1秒超シークではキャッシュを破棄する。停止中の手動シーク・「モザイクを適用」・動画書き出しは各フレームの高品質SAMを使う。
- **キーフレーム補間（v0.0.00150〜）**: `VideoEditState.interpolatedKeyframe(at:)` は前後キーフレームに同じIDで存在するROIの矩形、回転、信頼度を線形補間する。新規登場ROIは次キーフレームまで先読み表示せず、消失ROIは次キーフレームまで維持する。追跡側の正規キーフレーム選択 `keyframe(at:)` は変更しない。
- **追跡コンテキスト（v0.0.00150〜）**: `VideoROITracker` は性器ROIを1.8倍、その他を1.45倍へ広げた周辺矩形でVision追跡し、追跡結果から元ROIの相対位置だけを復元する。小さく質感の少ない部位単体を追うより周辺の肌・衣服の特徴を利用でき、モザイク範囲自体は拡大しない。再検出統合はIoUに加えて中心距離・サイズ比を使い、複数候補はスコア順の1対1対応にする。
- **動画ショートカット（v0.0.00150〜、v0.0.00152更新）**: 動画編集中のみ、`Cmd+<`/`Cmd+>`と左右カーソルキーを1フレーム前後、`<`/`>`をキーフレーム前後、Spaceを再生/一時停止、`Cmd+K`を追加、`Cmd+Option+K`を一覧選択削除へ割り当てる。テキスト入力中は文字キーを横取りしない。キーフレーム一覧が選択中でも左右キーは1フレーム前後へ移動し、Enterは選択行のキーフレーム時刻を開く。
- **動画設定の同期（v0.0.00149）**: 自動解析ROIには、検出時点の `maskEngineRawValue` と `maskThreshold` を埋め込む。全体のモザイク種類を変更した場合は、旧既定スタイルを持つ動画キーフレームROIだけ新しい既定スタイルへ更新し、個別スタイルは保持する。全体のマスク生成方式・形状しきい値を変更した場合は、個別設定OFFの全体適用として動画キーフレーム全体へ同期する。
- **フレーム手動修正（v0.0.00151〜、v0.0.00152時刻同期）**: 動画キャンバスのROI編集完了時は、現在フレームを手動キーフレームとしてサイドカーへ即時保存する。停止中シーク、一時停止、保存直前の時刻は元動画fpsの最寄りフレーム境界へ揃え、表示したデコードフレームと保存先時刻を一致させる。修正前の補間位置との差分（中心・サイズ・回転）は前後最大1秒の自動/追跡キーフレームへ線形減衰で反映し、別の手動キーフレームを境界として上書きしない。形状・カテゴリ・マスクペン・個別設定・削除・Undo/Redoも現在フレームへ保存するが、幾何差分が無い変更は近傍へ伝播しない。自動解析再実行時も手動修正を時刻順に復元して同じ周辺補正を再適用する。修正手順は、左右キーまたは`Cmd+<`/`Cmd+>`で対象フレームへ移動し、ROI/レイヤ/モザイクを直接編集する。編集完了時に自動保存されるため`Cmd+K`は不要。1秒を超える区間、シーンカット、遮蔽の前後は追加の手動修正アンカーを置く。
...
導入当初はツールバーの3つ目のセグメント（筆アイコン）1つで「マスク修正モード」に切り替え、
Option(⌥)キーで塗り/消しを反転する設計だった。v0.0.00119で「マスク追加ペン」「マスク消しゴム」の
2モードへ分割している（Optionによる一時反転も維持。詳細は§5.61）。

- 選択中のレイヤ上をドラッグ → 塗る（マスク追加ペン）／消す（マスク消しゴム）
- Option(⌥)キーを押しながら → 動作を一時的に反転
- 太さはインスペクタの「ペンの太さ」（マスク追加ペン／マスク消しゴムのいずれかのときだけ表示）
- 1ストロークが1回の「元に戻す」単位
...
### 5.61.3 カーソルとタブレット入力のレスポンス（v0.0.00138）

マスク追加ペン/消しゴム中は、画像上のROI・人物・骨格レイヤにヒットしていても移動操作の
openHandカーソルを出さず、ペン/消しゴムのカーソルを優先する。Option(⌥)で塗り/消しを反転した
場合もカーソルを即時更新する。

タブレットペンのdragイベントは高頻度に発生するため、`ImageCanvasView` はドラッグ中のストロークを
軽量なビュー上プレビューとして描画し、確定時だけ `ManualMaskStroke` としてROIへ保存する。再描画は
筆跡周辺のdirty rectに限定し、画面上1.25pt未満の連続点は間引く。これにより、手描き補正の保存形式
（ストローク保持）とUndo単位を変えずに、入力中の視覚反応を改善する。

### 5.61.4 ログ消去が効かなかった原因
...
## 5.66 SAMの部分的な検閲漏れとしきい値の較正（2026-08-03 v0.0.00123〜）
...
**未検証の範囲（情報共有）**: `minimumUsableCoverage` は `SAMSegmentEngine` クラス全体で共有する
定数であり、性器以外にSAMを使う場合（本ファイル冒頭のクラスコメントの通り、乳首もSAMの対象）にも
同じ0.40が適用される。今回の実測は性器ROI（男性器・女性器）11件に限定しており、乳首・乳輪ROIを
SAMへ通した場合に被覆率0.40未満で不要にフォールバックしないかは未確認（サンプル画像には乳首・
乳輪ROIも含まれるが、回帰テストは性器カテゴリのみでフィルタしている）。フォールバック自体は
矩形全体を塗る安全側の動作なので検閲漏れにはならないが、乳首のマスクが必要以上に矩形化される
「品質の低下」が起きる可能性がある。次に乳首・乳輪でSAMを使う場合は、このしきい値がカテゴリ別に
分けるべきかどうかを検討すること。
...
## 5.67 SAM経路の二段フォールバック（2026-08-06 v0.0.00130〜）

§5.66の較正後、8C52ADAB右下の男性器ROI（full経路 coverage=0.26）は図形フォールバック
（楕円全塗り）になっていたが、「形状に沿ったマスクが生成されない」とのGUI報告を受けて再調査。
同じROIを**crop経路（切り出し推論）**へ通すと coverage=0.41 でペニス輪郭に沿ったマスクが
得られることを実測（PNGレンダリングで目視確認）した。

- `samMask` の経路選択を「第一経路（ROIサイズで full/crop を選択）→ 不合格なら反対経路 →
  それでも不合格なら図形フォールバック」の二段チェーンへ変更。
- 反対経路の採用条件は「`minimumUsableCoverage`(0.40) 以上 かつ 第一経路より高い」。
  しきい値は§5.66の較正のまま変えていないため、検閲漏れ側の安全性は同一。
- `SegmentMask` ログの `path=` に `full→crop` / `crop→full` が出た場合はフォールバック採用を示す。
- 回帰テスト `lowCoverageSAMMaskFallsBackToShape` は「このROIはcrop経路の形状追従マスク
  （0.40≦coverage<0.95）になる」期待へ更新した。

## 5.68 samShapeマスクの安全側膨張（2026-08-07 v0.0.00131〜）

GUI報告「ディルド範囲ずれ」（7624B71B・実写）の実測調査:
- 同一画像（md5一致）・同一ROI・アプリ同一経路の再現レンダリングで、SAMマスクは対象に
  正確に一致し、パイプラインに系統的な位置ズレは無いことを確認（coverage 0.596 ＝報告ログの0.60）。
- 報告の実体は (a) SAMマスクが対象輪郭ぎりぎりで止まり、明るく写る縁（ハイライト部）が
  数px露出する非対称な縁の欠け、(b) ユーザーがROIをドラッグしても、SAMは枠を少し動かしても
  同じ対象へロックするためマスクが追従せず「ずれたまま」に見える、の2点。

対策: `samMask` でSAMマスクへ**ROIサイズの3%（2〜12px）の膨張（`CIMorphologyMaximum`）**を
形状クリップの前に適用する。検閲ツールでは覆い不足が最悪の失敗であり、縁スライバー露出を
...
## 5.70 AppKitメニューと動画SAM補正の永続化（2026-08-23 v0.0.00153〜）

- メインメニューのトップレベル`NSMenuItem`は、サブメニュー任せにせず明示タイトルを持つ。
  全階層と`windowsMenu`/`helpMenu`は`applicationDidFinishLaunching`で差し替えず、`app.run()`前に確定して
  AppKitのメニューバー追跡領域とプルダウン位置の基準を一致させる。
- インスペクタ「画像種別」の`NSPopUpButton`は項目生成時にtarget/actionを設定し、
  `menu.autoenablesItems = false`として自動判定／実写／イラスト・漫画を常時選択可能にする。
- 性器ROIのSAMプロンプトは中心正例点(label 1)＋box角(label 2/3)とし、枠内の背景や人体側を
  選ぶ誤分離を抑える。人物シルエット等のbox-only経路は変更しない。
