...
- **mainSplitViewのdivider index規約**（重要）: `mainSplitView` は常に「左ペイン(0)/キャンバス(1)/右ペイン(2)」の3 arrangedSubviewsを持ち、`isHidden` による非表示状態でもインデックスは詰め直されない（`NSSplitViewDelegate` の `constrainMinCoordinate`/`constrainMaxCoordinate` 実装が `subviews[dividerIndex]`/`subviews[dividerIndex+1]` を直接参照している通り）。divider 0 = 左ペイン/キャンバスの境界、divider 1（= `arrangedSubviews.count - 2`）= キャンバス/右ペインの境界。既定では左ペインが非表示のため、**右ペイン幅を操作する箇所は必ず`arrangedSubviews.count - 2`を使う**こと。固定の`0`を使うと（左ペインが非表示のときは特に）無効な境界を操作するだけで右ペイン幅に反映されない（Build 71で `applyInitialLayoutIfNeeded()` の既定レイアウト処理に残っていた同種のバグを修正）。
