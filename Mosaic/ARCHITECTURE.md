...
- 画像上に重畳するレイヤは「画像」「モザイク対象（ROI）」の固定2種類と、候補生成の都度動的に生成される「人物検出N」「骨格検出N」（N=検出順の番号）。個別に表示/非表示を切替できる（ツールバーの「人物検出レイヤ」「骨格検出レイヤ」チェックボックスは全人物・全骨格の一括ON/OFF、または常設レイヤパネルで個別制御）。
