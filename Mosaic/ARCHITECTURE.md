...
- `PoseEstimator`（`VisionPoseEstimator`）: `VNDetectHumanBodyPoseRequest`（複数人物対応）で15関節を取得し、**シルエットマスク内に入る関節数を主基準・中心距離をタイブレークとする貪欲マッチング**（`PersonMaskSampler` による縮小マスク参照）で人物へ対応付ける（Build 26で改善。人物矩形の重なりが大きい構図での誤対応を防止。マスクを持つ人物に関節が1つも入らない骨格は割り当てない）。関節は `PoseJoint`（正準名enum・画像正規化座標・confidence）として `PoseHint.joints` に保持。骨格が得られない人物には `HeuristicPoseEstimator` の固定比率フォールバックヒントを適用（ただしROIは関節必須のため候補は生成されない）。
