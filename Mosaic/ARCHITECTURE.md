...
- **mainSplitViewの右ペイン幅設定（重要・Build 72で方針転換）**: `mainSplitView` は「左ペイン(0)/キャンバス(1)/右ペイン(2)」の3 arrangedSubviewsを持つが、既定で左ペインは非表示であり、非表示のarrangedSubviewに対して `NSSplitView.setPosition(_:ofDividerAt:)` へ渡すべきdivider indexが「詰めずに数えた`arrangedSubviews.count - 2`」なのか「非表示分を詰めて数え直した`0`」なのかは、Build 69・71の2回にわたり異なる仮説で修正を試みたが実機確認できず特定できなかった（`NSSplitViewDelegate`の`constrainMinCoordinate`/`constrainMaxCoordinate`実装は前者の解釈で書かれているが、Appleの公式ドキュメントは後者を示唆しており、実機での再現結果は両方の修正を無効化するように見えた）。そのため、**右ペイン幅を設定する箇所はどちらのindexが正しいかを決め打ちせず、`setMainSplitRightPaneWidth(_:mainSplit:rightPane:)`（候補indexを順に試し、実際に反映された幅を測定して確定する自己補正方式）を必ず使う**こと。`restoreSplitPositions()`・`applyPaneWidth()`・`applyPanelAssignments()`・`applyInitialLayoutIfNeeded()` の右ペイン幅を扱う箇所はすべてこの関数へ統一済み。新たに右ペイン幅を設定するコードを追加する場合も、直接`setPosition(_:ofDividerAt:)`を呼ばずこの関数を使うこと。
