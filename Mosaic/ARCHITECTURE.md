...
- `ROIGenerator`（`SensitiveROIGenerator`）: 関節からの解剖学的プライアでカテゴリ付きROIを人物ごとに生成。胸部（乳首）=肩幅・肩-腰距離から左右2点（category `.nipple`）、鼠径部=腰中心から膝方向へ `groinPositionRatio`（既定0.45、UIスライダーで20〜80%へ事前補正可・UserDefaults永続化）のオフセット（category `.other`。性別分類器が未導入のため男女の判別は保留、Phase 2で対応予定）。関節が無い場合は従来の下半身固定比率（source `heuristic-lower-body`）。
