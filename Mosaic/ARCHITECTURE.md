...
- **動画ショートカット（v0.0.00150〜）**: 動画編集中のみ、`Cmd+<`/`Cmd+>`を1フレーム前後、`<`/`>`をキーフレーム前後、Spaceを再生/一時停止、`Cmd+K`を追加、`Cmd+Option+K`を一覧選択削除へ割り当てる。テキスト入力中は文字キーを横取りしない。キーフレーム一覧が選択中なら左右キーは選択行基準の前後移動、Enterは選択行の時刻を開く。
...
- **フレーム手動修正（v0.0.00151〜）**: 動画キャンバスのROI編集完了時は、現在フレームを手動キーフレームとしてサイドカーへ即時保存する。修正前の補間位置との差分（中心・サイズ・回転）は前後最大1秒の自動/追跡キーフレームへ線形減衰で反映し、別の手動キーフレームを境界として上書きしない。形状・カテゴリ・マスクペン・個別設定・削除・Undo/Redoも現在フレームへ保存するが、幾何差分が無い変更は近傍へ伝播しない。自動解析再実行時も手動修正を時刻順に復元して同じ周辺補正を再適用する。
