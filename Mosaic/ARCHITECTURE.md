...
- **動画自動モザイク処理**: 現在の候補カテゴリ/検出設定を使い、動画全体を詳細設定の再検出間隔に基づくサンプル間隔で解析する。ROIが検出された時刻だけをキーフレームとして保存し、空のキーフレームを大量に作らない。これは一括キーフレーム生成であり、実際のモザイク済み動画ファイルは従来どおり `VideoMosaicExporter` の書き出しで生成する。
...
- **再生パフォーマンス（v0.0.00148〜v0.0.00149）**: 動画再生中は手動シークと同じ高品質経路を毎フレーム同期実行しない。専用キューで近傍フレーム取得と軽量モザイク合成を行い、メインスレッドでは表示反映と時刻ラベル更新だけを行う。処理中に次の再生位置が来た場合は最新時刻だけを保持し、古いフレーム要求を積み上げない。v0.0.00149からは軽量プレビューでもROI個別の `maskEngine` を解決し、自動解析で保存した「対象形状（SAM）」指定を再生中の追随モザイクへ反映する。手動シーク・「モザイクを適用」・動画書き出しでも同じROI個別設定を使う。
