...
- `PersonDetector`（`VisionPersonDetector`）: `VNGeneratePersonInstanceMaskRequest` により**人物ごとの実位置**（インスタンスラベルマップからの外接矩形）と**人物シルエットマスク**（CGImage）を取得する。最大4人。失敗時・5人以上は `VNDetectHumanRectanglesRequest` へフォールバック。固定比率矩形は廃止し、検出0件時は空配列を返す（候補なし）。
