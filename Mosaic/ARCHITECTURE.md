...
- `PersonDetector`（`VisionPersonDetector`）: `VNGeneratePersonInstanceMaskRequest` により**人物ごとの実位置**（インスタンスラベルマップからの外接矩形）と**人物シルエットマスク**（CGImage）を取得する。最大4人。失敗時・5人以上は `VNDetectHumanRectanglesRequest`（revision 2・全身矩形）へフォールバック。**両方0件の場合は従来互換の中央人物想定矩形を1件返す**（Build 14で復活。漫画・イラスト等で実写学習のVisionが反応しない場合に候補が全滅しないための保証）。
