...
- **保存ルール**: 動画編集中の「モザイクを適用」は、現在時刻のROIを保存時点のモザイク設定で固定し、`VideoEditStore` のキーフレームとして保存する。静止画の `saveProcessedImage` は呼ばない。自動処理・追跡・未保存確認保存も同じ `VideoEditState.resolvingInheritedStyles` 系の処理で、未設定ROIへ保存時点のスタイルを埋める。
- **再生パフォーマンス（v0.0.00148）**: 動画再生中は手動シークと同じ高品質経路を毎フレーム同期実行しない。専用キューで近傍フレーム取得と図形マスクによる軽量モザイク合成を行い、メインスレッドでは表示反映と時刻ラベル更新だけを行う。処理中に次の再生位置が来た場合は最新時刻だけを保持し、古いフレーム要求を積み上げない。手動シーク・「モザイクを適用」・動画書き出しでは従来通り選択中のマスク生成方式を使う。
